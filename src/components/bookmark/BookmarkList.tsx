...
export const BookmarkList: React.FC<BookmarkListProps> = ({ bookmarks, collection }) => {
    ...
    const openLinkOnCurrentPage = (link: string) => { window.open(link, '_self') }
    const openLinkMiddleClick = (event: any, link: string) => { if (event.button === 1) { window.open(link, '_blank') } }
        ...
        <Row>
            <Col span={24} >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <Typography.Title level={4} style={{ marginBottom: 0 }}>{collection.icon} {collection.name}</Typography.Title>
                    <Space>
                ...
                {bookmarks && bookmarks.length > 0 && viewMode === BookmarkViewMode.LISTE && (
                    <List header={"test"} dataSource={bookmarks} renderItem={(item: Bookmark) => (
                        <BookmarkContextDropdrown bookmark={item}>
                            <StyledListItem
                                onClick={() => { openLinkOnCurrentPage(item.link) }}
                                onMouseDown={(event: any) => { openLinkMiddleClick(event, item.link) }}
